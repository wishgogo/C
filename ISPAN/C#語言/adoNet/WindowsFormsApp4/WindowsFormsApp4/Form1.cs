...
namespace WindowsFormsApp4
{
    ...
    {
        SqlConnectionStringBuilder scsb; //SqlCommandBuilder SQL連接字串建立器
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            cbx欄位名稱.Items.Add("姓名");
            cbx欄位名稱.Items.Add("電話");
            cbx欄位名稱.Items.Add("email");
            cbx欄位名稱.Items.Add("地址");
            cbx欄位名稱.SelectedIndex =0;

            radio婚姻全部.Checked = true;
            int搜尋婚姻狀態 = 0; //全部:0,已婚1,未婚2
        ...
        private void btn修改資料_Click(object sender, EventArgs e)
        {
            ...
            if((intID > 0) && (txt姓名.Text!="") && (txt電話.Text!=""))
            {
                ...
                int rows = cmd.ExecuteNonQuery(); //回傳異動資料筆數
                con.Close();
        ...
        private void btn新增資料_Click(object sender, EventArgs e)
        {
            if((txt姓名.Text!="")&&(txt電話.Text!=""))
            {
                ...
                SqlCommand cmd = new SqlCommand(strSQL,con);
                //insert into persons values('David Wang', '0922999111', '台北市內湖區港湖路', 'davidwang@gmail.com', '1977-3-4', 1, 2800);
                cmd.Parameters.AddWithValue("@NewName",txt姓名.Text);
                cmd.Parameters.AddWithValue("@NewPhone", txt電話.Text);
                cmd.Parameters.AddWithValue("@NewAddress", txt地址.Text);
                cmd.Parameters.AddWithValue("@newEmail", txtEmail.Text);
                ...
                Int32.TryParse(txt點數.Text,out int點數);
                cmd.Parameters.AddWithValue("@NewPoints",int點數);
                //cmd.Parameters.AddWithValue("@NewPoints", Convert.ToInt32(txt點數.Text));
        ...
        private void btn搜尋_Click(object sender, EventArgs e)
        {
            ...
            searchIDs.Clear();
            string str欄位名稱 = cbx欄位名稱.SelectedItem.ToString();
            ...
            if (txt欄位關鍵字.Text != "")
            {
                string strSQL = "select * from persons where (" + str欄位名稱 + " like @SearchName) and (生日 <= @EndDate and 生日 >= @StarDate)" + strSQL婚姻狀態查詢語法;
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand(strSQL ,con);
        ...
        //cell =儲存格 e=事件本身 sender=觸發事件
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex>=0)
            {
                string strselID = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();//[e.RowIndex]=選擇的資料列索引 cells[0]=資料欄位索引=ID
                int intSelId = 0;
